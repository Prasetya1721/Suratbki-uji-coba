  ...
  useEffect(() => {
    if (editItem) {
      ...
      setFormData({
        ...
        petugas: defaultSurat?.petugas || ((role === 'surveyor' || role === 'kacab' || role === 'kacap') ? (currentUser?.name || '') : ((usersList?.filter(u => u.role === 'surveyor' || u.role === 'kacab')[0]?.name) || 'ALFIAN BONE PUTRA')),
              ...
              >
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                  ...
                  <div style={{ background: 'var(--bg-card-solid)', padding: '0.85rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)' }}>
                    ...
                    <MultiDocUpload
                      value={formData.fileTiketTransportData || formData.fileTiketTransportName}
                      onChange={(val) => setFormData((prev) => ({ ...prev, fileTiketTransportName: val, fileTiketTransportData: val }))}
                  ...
                  <div style={{ background: 'var(--bg-card-solid)', padding: '0.85rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)' }}>
                    ...
                    <MultiDocUpload
                      value={formData.fileKwitansiHotelData || formData.fileKwitansiHotelName}
                      onChange={(val) => setFormData((prev) => ({ ...prev, fileKwitansiHotelName: val, fileKwitansiHotelData: val }))}
                      onPreview={setPreviewAttachment}
